...
GOI <- df_DE %>%
  filter(symbol %in% vect2) %>%
  filter(abs(log2FoldChange)>1,padj<0.05) %>%
  pull(symbol)

df_DE %>%
  filter(symbol %in% c("KDR"))
...
pdf("../../out/image/heatmap_GOI2.pdf",width = 6,height = 6) 
...
ggsave("../../out/image/boxplot_GOI2.pdf",width = 10,height = 10) 
...
counts(data,normalized=T)%>%
  ...
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill = NA))
ggsave("../../out/image/scatterplot_GOI2.pdf",width = 15,height = 10) 
