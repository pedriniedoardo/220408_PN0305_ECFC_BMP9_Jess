...
df_tables_GSEA_all %>%
  write_tsv("../../out/table/df_tables_GSEA_res_BMP9_vs_Mock_shr_all_REACTOME.tsv")
...
# save the table
df_tables_GSEA_all_non_redundant %>%
  write_tsv(file = "../../out/table/df_table_GSEA_res_BMP9_vs_Mock_shr_nonredundant_REACTOME.tsv")
...
# plot_pathway <- lapply(df_tables_GSEA_all$pathway[1:9],function(x){ 
#   name <- str_sub(x,start = 1,end = -4) 
#   # pdf(file = paste0("HUVEC_BMP9_24h_",name,".pdf"),width = 3,height = 3) 
#   plotEnrichment(pathways[[x]], list_ranks$res_sen_final_shr) + labs(title = name) 
# })

wrap_plots(plot_pathway)
ggsave(filename = "../../out/image/GSEA_plot_profile_BMP9_vs_Mock_shr_nonredundant_REACTOME.pdf",width = 15,height = 10)
