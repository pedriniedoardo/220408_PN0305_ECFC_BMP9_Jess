...
df_GSEA %>%
  filter(str_detect(pathway,pattern = "WNT"))
...
pdf("../../out/image/heatmap_GOI_WNT.pdf",width = 6,height = 4) 
...
ggsave("../../out/image/boxplot_GOI_WNT.pdf",width = 9,height = 6) 
...
counts(data,normalized=T)%>%
  ...
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill = NA))
ggsave("../../out/image/scatterplot_GOI_WNT.pdf",width = 10,height = 6) 
